...
Full ARM core dump source code using Keil uVision is attached to the article. The source was written using the free Keil MDK-Lite and ARM STM32F103 simulator so no target hardware was required. The code is simple and easily ported. 
...
```
A pragma is used to place an instance of the structure within the NoInit section. This prevents zero initialization of the data when the CPU starts. 
```cpp
...
# CoreDump Project
The CoreDump Keil ARM project fully implements the core dump feature, except the crash data persistence. A software assertion and a hardware exception are demonstrated. A few source files contain the key elements:
1. Main.c
2. CoreDump.c/h
3. CoreDumpData.h
4. Fault.c/h
...
```
`HardFault_Handler()` is called when a hardware exception occurs and is called from an interrupt context. For an ARM architecture, during an interrupt context the stack pointer is located in the MSP or PSP register. Once the address of the stack is acquired, the core dump is stored and the CPU reset. 
```cpp
...
static void StoreThreadCallStacks()
{
    ...
    memset(_crashDumpData.OS, 0, sizeof(_crashDumpData.OS));
    ...
    for (INT t=0; t<=OS_TASKCNT; t++)
    {
        ...
        StoreCallStack((unsigned int*)p_TCB->tsk_stack, &_crashDumpData.CallStack[taskCnt][0], CALL_STACK_SIZE);
